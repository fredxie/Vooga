...
public class HUD {
	...
	public HUD(Graphics2D g, String font, String title, String type,
			Object para, int x, int y) {
		myFont = font;
		myTitle = title;
		myInfoType = type;
		myObject = para;
	...
	public void displayMono() {
		if (myObject == null) {
			fontManager.getFont(myFont)
					.drawString(myGraphic, myTitle, myX, myY);
		} else {
			InfoFactory info = new InfoFactory(myInfoType, myObject);
			fontManager.getFont(myFont).drawString(myGraphic,
					myTitle + info.getInfo(), myX, myY);
		}
	}
