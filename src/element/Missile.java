...
public class Missile extends Bullet{
	...
	public Missile(BufferedImage image, double x, double y, double eNEMY_WEAPON_DAMAGE) {
		super(image, x, y);
	...
	public Bullet[] genBullets(Fighter fighter, int numOfBullet) {
		...
		case 0: {
			...
			missile[0] = new Missile(image, fighter.getX() + fighter.getWidth() / 2,
					fighter.getY() - 20, damage);
			missile[0].setVerticalSpeed(-0.7);
			break;
		}
		case 1: {
			for (int i = 0; i < 3; i++) {
				missile[i] = new Missile(image,
						fighter.getX() + fighter.getWidth() / 2, fighter.getY() - 20,
						damage);
