...
public class SpawnByLocation implements SpawnBehavior {
	 ...
	 * 
	 * @param Y_Location
	 *            : this parameter means you should input a decimal value
	 *            (between 0 and 1) to indicate the approximate position on the
	 *            entire map of the group of enemy 0 means at the top of the map
	 *            and 1 means at the bottom of the map. If the input is larger
	 *            than 1, the program only extracts its decimal part.
	 */
	 ...
	 * 
	 * @param scale
	 *            means the range in y coordinate
	 * @param waveNumber
	 *            represents i-th wave
	 * @param totalWave
	 *            represents the total number of waves
	 * @return the Y coordinate or enemy
	 * @Description use Normal distribution in each wave to calculate Y
	 *              coordinate
	 */
	...
	//
	// double
	// groupPosition=(distanceBetweenWave*waveNumber+(1-scale)/2);//Return the
	// mean position of this group
	//
	// double
	// relativePosition=(NormalDist.inverseF01(Math.random())/6)*distanceBetweenWave;
	//
