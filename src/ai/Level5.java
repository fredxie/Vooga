...
public class Level5 implements TopDownBehavior {
		...
		timer1 = new Timer(400, new ActionListener() {
			public void actionPerformed(ActionEvent e1) {
				if (e1.getSource().equals(timer1)) {
					if (rand < 10) {
						enemy.setSpeed(Math.random() * 0.26,
								Math.random() * 0.26);
	...
	public void weaponSpeed(Missile missile) {
		/*
		 * in this level, the hypothetical final level on hard, bullet speed/
		 * direction is random, and bullets stay in frame, they dont exit frame
		 * so player must dodge them Missile update needs to be moves to
		 * gameStateLevel_ like enemy movement bounds were
		 */
		h = Math.random() * 0.25;
		v = Math.random() * 0.35
		...
		if (Math.random() * 51 < 25) {
			...
			if (missile.getX() <= 0
					|| missile.getX() >= DemoGameEngine.WIDTH
							- ((missile.getWidth()))) {
				missile.setSpeed(-h, v);
			}
		} else {
